...
describe("GET /api/articles", () => {
  it("should fetch articles with authors successfully", async () => {
    const response = await request(app).get("/api/articles");

    // Ici je vérifie que la réponse est un tableau et que le status est 200
    expect(response.status).toBe(200);
    expect(response.body).toBeInstanceOf(Array);

    // si ma variable response est un tableau et qu'il contient au moins un élément , alors je teste les propriétés de cet élément
    if (response.body.length > 0) {
      const firstArticle = response.body[0];
      // Vérifie la présence du titre de l'article
      expect(firstArticle).toHaveProperty("titre");
      // Idem pour contenu
      expect(firstArticle).toHaveProperty("contenu");
      // Idem pour l'auteur
      expect(firstArticle).toHaveProperty("auteur_pseudo");

      // Ici je vérifie que les propriétés ne sont pas vides
      expect(firstArticle.titre).toBeTruthy();
      expect(firstArticle.contenu).toBeTruthy();
      expect(firstArticle.auteur_pseudo).toBeTruthy();
    }
  });
});
